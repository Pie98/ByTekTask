...
if __name__ == "__main__":
    ...
    test_data_valid = {
        "user_id": "user123",
        "features": {
            "tempo_ultimo_acquisto": 45.0,
            "frequenza_visita_30gg": 15.0,
            "percentuale_abbandono_carrello": 0.3,
            "valore_medio_carrello": 89.50,
            "orario_preferenziale": 14.0
        }
    }
    ...
    test_data_invalid = {
        "user_id": "user456",
        "features": {
            "percentuale_abbandono_carrello": 1.5,  # Errore: > 1
            "orario_preferenziale": 25.0,  # Errore: > 23
            "frequenza_visita_30gg": 500.0,  # Warning: alto ma possibile
            "tempo_ultimo_acquisto": -5.0  # Errore: negativo
        }
    }

    print("=== Test dati validi ===")
    is_valid, data, errors, warnings = service.validate_request(test_data_valid)
    print(f"Valido: {is_valid}")
    print(f"Errori: {errors}")
    print(f"Warning: {warnings}")

    print("\n=== Test dati invalidi ===")
    is_valid, data, errors, warnings = service.validate_request(test_data_invalid)
    print(f"Valido: {is_valid}")
    print(f"Errori: {errors}")
    print(f"Warning: {warnings}")